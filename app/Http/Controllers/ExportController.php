...
class ExportController extends Controller
{
    ...
    public function deptPhdUnSelCandidatesExport($dept)
    {
        ...
        Excel::create($dept.'_phd_not_selected'.$phdCandidates[0]['created_at'], function($excel) use($phdCandidates) {
            $excel->sheet('Sheet 1', function($sheet) use($phdCandidates) {
                $sheet->fromArray($phdCandidates);
            });
        })->export('xlsx');
    ...
    public function deptMsUnSelCandidatesExport($dept)
    {
        ...
        Excel::create($dept.'_ms_not_selected'.$msCandidates[0]['created_at'], function($excel) use($msCandidates) {
            $excel->sheet('Sheet 2', function($sheet) use($msCandidates) {
                $sheet->fromArray($msCandidates);
            });
        })->export('xlsx');
